...
debug_skip_mapping=False #calculate mapping will take 1 min or more, very unfriendly for debugging
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str ,default='v2_64_highwd2', help='model path')
    ...
    _, _, groupc, mlpc, mlpc2, mapmax = model_param
    assert(mlpc==mlpc2)
...
#-------------------------------------------------------------
    ...
    buf=np.ones((4*(3**mapL),groupc*2),dtype=np.float64)*114514

    pow3 = np.array([3**i for i in range(mapL+1)], dtype=np.int64)
    pow3=pow3[:,np.newaxis]
    #无边界和单边边界 长度11

    data=fullData(mapL)
    if not debug_skip_mapping:
        label=model.exportMapTable(data,device)

        #无边界与正边界
        for r in range(mapHalfL+1):
            ids=np.matmul(data[:,r:],pow3[:mapL-r])+pow3[mapL+1-r:].sum()
            for i in range(ids.shape[0]):
                buf[ids[i]]=label[:,i,mapHalfL+r]

        #负边界
        for l in range(1,mapHalfL+1):
            ids=np.matmul(data[:,:mapL-l],pow3[l:-1])+2*3**mapL+pow3[:l-1].sum()
            for i in range(ids.shape[0]):
                buf[ids[i]]=label[:,i,mapHalfL-l]

        #双边边界
        for left in range(1,mapHalfL+1):
            for right in range(1,mapHalfL+1):
                L=mapL-left-right
                data = fullData(L)
                label=model.exportMapTable(data,device)
                idbias=3*3**mapL+pow3[0:left-1].sum((0,1))+pow3[mapL+1-right:-1].sum((0,1))
                ids=np.matmul(data,pow3[left:mapL-right])+idbias
                for i in range(ids.shape[0]):
                    buf[ids[i]]=label[:,i,mapHalfL-left]
    ...
    if(debug_skip_mapping):
        bound=mapmax
    bound+=mapmax*2 # h1 = g1.mean(1)+gfVector(gf)+illegalBias,  |gfVector(gf)|<30, |illegalBias|<30
    print("Max=",bound)
    ...
    map_maxint=8000
    g2extrascale=0.8
    trunkconv1_extrabound=30 #允许少量超越bound
    trunkconv2_extrabound=100
    ...
    # 获取需要处理的行索引
    useful_indices = np.where(useful)[0]

    # 提前计算两组缩放值
    scaled_w = (buf[:, :groupc] * w_scale).astype(int)
    scaled_g2 = (buf[:, groupc:groupc*2] * g2scale).astype(int)

    # 缓存结果到列表
    results = []
    for i in useful_indices:
        row = [f"{i}"] + scaled_w[i].tolist() + scaled_g2[i].tolist()
...
# illegal vector
    ...
    w = model.illegalVector.data.cpu().numpy().flatten()
    # pytorch里面是4线平均，c++里是4线求和，所以需要额外乘4
    w[:groupc]=w[:groupc]*scale_now*4
    w[groupc:]=w[groupc:]*g2scale*4
    maxint=np.abs(w).max()
    maxint=max(maxint,np.abs(b).max())
    print(f"illegal vector max = {maxint}")
    print("illegalVector", file=exportfile)
    for i in range(2*groupc):
        print(int(w[i]), end=' ', file=exportfile)
    print('', file=exportfile)
...
#2 h1conv
    ...
    assert(h1conv_w.shape==(mapL,groupc))
    ...
    for i in range(mapHalfL+1):
        for j in range(groupc):
            print(int(h1conv_w[i+mapHalfL,j]*2**15),end=' ',file=exportfile)
    print('',file=exportfile)
...
#6 trunkconv1
    ...
    w_scale = min(32700*trunkconv1_extrabound/bound,32700/maxint,140/scale_now)
...
#从这里往下的都是float 无需担心量化，且scale=1
    ...
    #mlp layer 4
    w=model.value_linear4.weight.data.cpu().numpy()
    b=model.value_linear4.bias.data.cpu().numpy()

    print("mlp_w4",file=exportfile)
    for i in range(mlpc):
        for j in range(mlpc):
            print(w[j][i],end=' ',file=exportfile)
    print('',file=exportfile)

    print("mlp_b4",file=exportfile)
    for i in range(mlpc):
        print(b[i],end=' ',file=exportfile)
    print('',file=exportfile)
