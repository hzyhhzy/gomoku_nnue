...
MCTSnode::MCTSnode(Evaluator * evaluator,
                   Color       nextColor,
                   double      policyTemp,
                   Loc *       locbuf,
                   PolicyType *pbuf1,
    ...
    : nextColor(nextColor)
{
  ...
  float k         = 1 / policySum;
  std::transform(pbuf3, pbuf3 + legalChildrennum, pbuf3, [=](auto &p) { return p * k; });
...
MCTSnode::MCTSnode(MCTSsureResult sureResult, Color nextColor)
    : nextColor(nextColor)
...
MCTSnode::~MCTSnode()
{
  for (int i = 0; i < childrennum; i++) {
    if (children[i].ptr() != NULL)
      delete children[i].ptr();
  }
}
...
float MCTSsearch::fullsearch(Color color, double factor, Loc &bestmove)
{
  ...
  // check VCF
  VCF::SearchResult VCFresult =
      vcfSolver[color - 1].fullSearch(10000, 10, bestmove, false);
  ...
  if (factor != 0)
    option.maxNodes = factor;
  terminate = false;

  // if root node is NULL, create root node
  if (rootNode == NULL) {
    rootNode =
        new MCTSnode(evaluator, color, params.policyTemp, locbuf, pbuf1, pbuf2, pbuf3);
  }
...
void MCTSsearch::play(Color color, Loc loc)
{
  ...
  if (rootNode != NULL) {
    ...
    if (rootNode->sureResult != MC_UNCERTAIN) {
      delete rootNode;
      rootNode = NULL;
    }
    //落子方不对，所以重新计算
    else if (rootNode->nextColor != color) {
      delete rootNode;
      rootNode = NULL;
    }
    //检查是否存在对应的子分支
      ...
      for (int i = 0; i < rootNode->childrennum; i++) {
        if (rootNode->children[i].loc == loc) {
          ...
          rootNode->children[i].setPtr(nullptr);  //防止被rootNode的析构函数删除。
          delete rootNode;
...
MCTSsearch::search(MCTSnode *node, uint64_t remainVisits, bool isRoot)
{
  if (remainVisits == 0)
    remainVisits = INT64_MAX;

  //正常情况是一次playout一个visit，为了降低开销直接让visit变成（1+expandFactor）倍，新增expandFactor倍的visit
  if (!isRoot)
    remainVisits =
        std::min(remainVisits, uint64_t(params.expandFactor * double(node->visits)) + 1);
  ...
  while (remainVisits > 0 && !terminate) {
    ...
    if (nextChildID >= node->childrennum)  // new child
    {
      ...
      if (sr != MC_UNCERTAIN)
        node->children[nextChildID].setPtr(new MCTSnode(sr, opp));
      else {
        playForSearch(color, nextChildLoc);
        node->children[nextChildID].setPtr(
            new MCTSnode(evaluator, opp, params.policyTemp, locbuf, pbuf1, pbuf2, pbuf3));
...
int MCTSsearch::selectChildIDToSearch(MCTSnode *node)
{
  ...
  if (childrennum < node->legalChildrennum) {
    double value =
        node->WRtotal / totalVisit - sqrt(totalChildPolicy) * params.fpuReduction;
    double policy         = double(node->children[childrennum].policy) * policyQuantInv;
    double visit          = 0;
    double selectionValue = MCTSselectionValue(puctFactor, value, visit, policy);
    if (selectionValue > bestSelectionValue)
      bestChildID = childrennum;
  }
...
Loc MCTSsearch::bestRootMove() const
{
  if (rootNode == NULL)
    return LOC_NULL;
  if (rootNode->legalChildrennum <= 0)
    return LOC_NULL;
  if (rootNode->childrennum <= 0)
    return rootNode->children[0].loc;
