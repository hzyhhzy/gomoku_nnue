...
struct Mix6weight_int16
{
  ...
  int16_t
      map_lr_slope_sub1div8[mix6::featureNum];  //减去1方便计算
                                                //0.25f(x)=x>>2+min(x,0)*slopeSub1Div8*2
  ...
  // 9  mlp
  float mlp_w1[mix6::valueNum]
              [mix6::valueNum];  // shape=(inc，outc)，相同的inc对应权重相邻
  float mlp_b1[mix6::valueNum];
  float mlp_w2[mix6::valueNum][mix6::valueNum];
  float mlp_b2[mix6::valueNum];
  float mlp_w3[mix6::valueNum][3];
  float mlp_w3_for_safety[5];  // mlp_w3在read的时候一次read
                               // 8个，会read到后续内存mlp_w3[mix6::valueNum-1][2]+5，
  float mlp_b3[3];
  float mlp_b3_for_safety[5];  // mlp_b3在read的时候一次read
