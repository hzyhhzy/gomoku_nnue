...
expand_node:
  ...
  for (int i = -1; i < BS * BS; i++) {
    ...
    if (isWin(me, move)) {
      ...
      vcfSolver[me - 1].playOutside(move, me, 1, true);
      vcfSolver[oppo - 1].playOutside(move, me, 1, true);
      ...
      vcfSolver[me - 1].undoOutside(move, 1);
      vcfSolver[oppo - 1].undoOutside(move, 1);
    }
