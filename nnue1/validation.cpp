...
void  main_validation(string modelpath, string datapath)
{
  ...
  assert(bf_npy.shape[2] == BS);
  assert(bf_npy.shape[3] == BS);
    ...
    for (int d1 = 0; d1 < 2; d1++) {
      for (int d2 = 0; d2 < BS; d2++) {
        for (int d3 = 0; d3 < BS; d3++) {
          bf1[d1 * BS * BS + d2 * BS + d3] =
              // bf[d3 * BS * 2 * N + d2 * 2 * N + d1 * N + i];
              bf[d1 * BS * BS + d2 * BS + d3 + i * BS * BS * 2];
        }
      }
    }
    float gf1[1];
